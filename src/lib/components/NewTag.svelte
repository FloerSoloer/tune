...
<section class="inline font-mono text-sm">
	...
	<div class="relative -me-2 hidden w-3xs [:checked~&]:block">
		<form class="absolute flex w-full gap-1 rounded-full bg-primary py-1.5 ps-4 pe-5">
			...
			<div class="flex grow flex-col justify-center gap-y-1">
				<label class="validator input input-xs input-ghost px-1.5 text-sm">
					<IconMdiTagOutline />
					<input name="name" autocomplete="off" placeholder="tag_name" required type="text" />
				</label>
				<label class="validator input input-xs input-ghost px-1.5 text-sm">
					<IconMdiBookshelf />
					<input name="cat" autocomplete="off" placeholder="tag_category" required type="text" />
				</label>
			</div>
		</form>
	</div>
